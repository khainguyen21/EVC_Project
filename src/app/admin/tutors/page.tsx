...
export default function ManageTutorsPage() {
  ...
  const handleDeleteAll = () => {
    if (tutors.length === 0) {
      showToast("There are no staff members to delete.", "error");
      return;
    }
    setConfirmModal({
      message: `This will permanently delete all ${tutors.length} staff member${tutors.length > 1 ? "s" : ""} and all their subjects and shifts. This cannot be undone.`,
      onConfirm: async () => {
        ...
        try {
          ...
          if (res.ok) {
            ...
            fetchTutors();
            showToast(
              `All ${deleted} staff member${deleted > 1 ? "s" : ""} deleted.`,
              "success",
            );
          } else {
            const errorData = await res.json();
            showToast(errorData?.error || "Failed to delete all.", "error");
          }
        } catch {
          showToast("Network error. Please try again.", "error");
        }
